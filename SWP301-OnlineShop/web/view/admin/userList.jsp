                    ...
                    <div class="row" style="margin-bottom: 10px;">
                        <table class="table table-hover">
                            ...
                            <tr>
                                ...
                                <td>${u.role.name}</td>
                                <td>
                                    <form action="user?action=changeStatus" id="changeStatus-${u.id}" method="post">
                                        <!--<input type="hidden" name="action" value="changeStatus">-->
                                        <input type="hidden" name="xpage" value="${page}">
                                        <input type="hidden" name="id" value="${u.id}">
                                        <c:if test="${!u.status}">
                                            <input type="hidden" name="newStatus" value="active">
                                            <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#active" onclick="openModal('changeStatus-${u.id}')">Unactive</button>
                                        </c:if>
                                        <c:if test="${u.status}">
                                            <input type="hidden" name="newStatus" value="unactive">
                                            <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#active" onclick="openModal('changeStatus-${u.id}')">Active</button>
                                        </c:if>
                                    </form>
                                </td>
                                <td>
